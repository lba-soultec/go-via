...
IMG = ghcr.io/tribock/souldeploy:$(VERSION)
...
kube_base_output_package = github.com/maxiepax/go-via
...
devrun-go:  ## Run a controller from your host.
	export LOG_LEVEL=DEBUG; go run -ldflags "-X main.commit=${VERSION} -X main.date=${TODAY}" *.go -file config.json
